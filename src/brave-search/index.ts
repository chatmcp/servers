...
const braveApiKey = getParamValue("brave_api_key");
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    const apiKey = braveApiKey || getAuthValue(request, "brave_api_key");
    if (!apiKey) {
      throw new Error("Error: BRAVE_API_KEY is required");
    }
    ...
    switch (name) {
      case "brave_web_search": {
        ...
        const results = await performWebSearch(apiKey, query, count);
        return {
      ...
      case "brave_local_search": {
        ...
        const results = await performLocalSearch(apiKey, query, count);
        return {
          content: [{ type: "text", text: results }],
          isError: false,
        };
